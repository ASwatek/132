...
handleLocalInput(struct db_entry** db)
{
  ...
  const char* instructions = " (a)dd, (l)ist, (q)uit, (f)ind \"name\", (r)emove \"name\", (e)xport to \"file\", or (i)mport from \"file\"\n";
  char buf[512];
  char buf2[512];
  ...
  for( ;; ) {
    ...
    // read input
    // : Uncomment the next line once you've implemented getALine()
    ...
    case 'q':
    case 'x':
      // : somehow get out of this run loop
      printf("Exited\n");
      return;
    ...
    case 'f': // FIND
      // :  This is part 6 of the lab (and is optional for this lab).  You'll need
    ...
    case 'i': // IMPORT
      // : implement this (similar to export)
      printf("name ? ");
      getALine(buf, 511, stdin);
      do_import_db(db, buf);
      break;
