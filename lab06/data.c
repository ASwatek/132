 ...
 */
void
dbe_print(struct db_entry* entry, FILE* file)
{
  //: Modify this function to have an additional FILE* parameter.
...
do_import_db(struct db_entry** db, const char* filename)
{
  ...
  char buf2[128];
  int count=0;
  //below: stores max 128b in buf up to =>, stores max 128b in buf2 after =>
  while(feof(fd)==0){
   fscanf(fd, "%128[^=]=>%128[^\n]\n", buf, buf2);
